...
func (ql *QRLoginProcess) Start(ctx context.Context) (*bridgev2.LoginStep, error) {
	...
	ql.Client = libgm.NewClient(
		libgm.NewAuthData(),
		nil,
		ql.User.Log.With().Str("component", "libgm").Str("parent_action", "qr pair").Logger(),
		ql.Main.br.GetHTTPClientSettings(),
	)
...
func (gl *GoogleLoginProcess) SubmitCookies(ctx context.Context, cookies map[string]string) (*bridgev2.LoginStep, error) {
	...
	gl.Client = libgm.NewClient(
		ad,
		nil,
		gl.User.Log.With().Str("component", "libgm").Str("parent_action", "google pair").Logger(),
		gl.Main.br.GetHTTPClientSettings(),
	)
