...
func (gc *GMClient) invalidateSession(ctx context.Context, state status.BridgeState, deletePairing, deleteCookies bool) {
	if deletePairing && deleteCookies {
		gc.Meta.Session = nil
	} else if deletePairing {
		gc.Meta.Session.ClearPairing()
	} else if deleteCookies {
		gc.Meta.Session.SetCookies(nil)
	} else {
		panic(fmt.Errorf("invalid invalidateSession call with both deletePairing and deleteCookies set to false"))
	}
	err := gc.UserLogin.Save(ctx)
	if err != nil {
